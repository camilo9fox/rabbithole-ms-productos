...
/**
 * Entidad que representa una miniatura (thumbnail) de un ítem en la base de datos.
 * Mapea a la tabla "thumbnails_item".
 */
@Entity
@Table(name = "thumbnails_item", 
       uniqueConstraints = {
...
public class ThumbnailItem implements Serializable {
     ...
     * La relación es Many-to-One (muchas miniaturas pueden pertenecer a un ítem del carrito).
     * Según la restricción CHECK, una miniatura debe tener o un ítem de carrito o un ítem de orden.
     */
    ...
    /**
     * ID del ítem del carrito, para mantener referencia explícita que se mapea a la columna de la BD
     */
     ...
     * La relación es Many-to-One (muchas miniaturas pueden pertenecer a un ítem de la orden).
     * Según la restricción CHECK, una miniatura debe tener o un ítem de carrito o un ítem de orden.
     */
    @ManyToOne(fetch = FetchType.LAZY)
    ...
    /**
     * ID del ítem de la orden, para mantener referencia explícita que se mapea a la columna de la BD
     */
     ...
     * La relación es Many-to-One (muchas miniaturas pueden tener el mismo tipo de ángulo).
     */
    @ManyToOne(fetch = FetchType.LAZY)
    ...
    /**
     * ID del tipo de ángulo, para mantener referencia explícita que se mapea a la columna de la BD
     */
     ...
     * La relación es Many-to-One (muchas miniaturas pueden apuntar al mismo recurso).
     */
    @ManyToOne(fetch = FetchType.LAZY)
    ...
    /**
     * ID del recurso en Cloudinary, para mantener referencia explícita que se mapea a la columna de la BD
     */
    @Column(name = "cloudinary_resource_id", insertable = false, updatable = false, nullable = false)
    private Long cloudinaryResourceId;

    /**
     * Verifica que el ítem cumple con la restricción de tener solo uno de los dos tipos de ítem asociado.
     */
    @PrePersist
    @PreUpdate
    protected void validateOnlyOneItem() {
        ...
        if (!(hasItemCarrito ^ hasItemOrden)) { // XOR operation - exactly one must be true
            throw new IllegalStateException("Un ThumbnailItem debe estar asociado exactamente a un ItemCarrito O un ItemOrden, pero no a ambos ni a ninguno.");
        }
    }
}
